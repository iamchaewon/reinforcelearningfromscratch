...
샘플 데이터를 얻을 때마다 평균을 구해야 할 때는 **증분 방식(Incremental Method)**이 더 효율적입니다.
...
class McAgent:
    ...
    def get_action(self, state):
        """현재 정책(self.pi)에 따라 행동 확률을 가져와 확률적으로 행동을 선택"""
        action_probs = self.pi[state]
        actions = list(action_probs.keys())
        probs = list(action_probs.values())
        return np.random.choice(actions, p=probs)
    ...
    def add(self, state, action, reward):
        """환경과의 상호작용 결과인 경험 (상태, 행동, 보상)을 메모리에 기록"""
        data = (state, action, reward)
        self.memory.append(data)

    def reset(self):
        """하나의 에피소드가 끝난 후, 메모리를 초기화"""
        self.memory.clear()

    def update(self):
        """몬테카를로 정책 제어의 핵심: Q 값 평가 및 정책 개선"""
...
# =========== Importance Sampling (균등 분포) ===========
# 행동 분포(b)를 사용하여 목표 분포(pi)의 기대값을 추정 
b = np.array([1/3, 1/3, 1/3])    # 균등 분포 (행동 정책)
samples = []
for _ in range(n):
...
| 중요도 샘플링 (균등 분포) | 2.95 | 10.63 | 높은 분산 |
| 중요도 샘플링 (개선) | 2.72 | 2.48 | 분산 감소, 정확도 향상 |
